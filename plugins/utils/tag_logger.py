...
async def grp_log(_, message: Message):
    ...
    log1 = f"""
âªï¸ #REPLIED
ð¤ <b>Replied by :</b> {sender_m}
ð¢ <b>ID :</b> <code>{sender_id}</code>
ð¥ <b>Group :</b> {message.chat.title}
ð <b>Message link :</b> <a href={message.link}>link</a>
ð¬ <b>Message :</b> â¬
"""
    log2 = f"""
#â£ #TAGS
ð¤ <b>Sent by :</b> {sender_m}
ð¢ <b>ID :</b> <code>{sender_id}</code>
ð¥ <b>Group :</b> {message.chat.title}
ð <b>Message link :</b> <a href={message.link}>link</a>
ð¬ <b>Message :</b> â¬
...
async def pm_log(_, message: Message):
    ...
    log = f"""
ð£ <b>#Conversation</b> with:
ð¢ <b>ID :</b> <code>{chat_id}</code>
ð¤ <a href="tg://user?id={chat_id}">{chat_name}</a> â¬
"""
    try:
        me_id = user(info="id")
        if sender_id == me_id:
