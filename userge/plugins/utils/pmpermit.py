...
async def pmguard(message: Message):
    ...
    if Config.ALLOW_ALL_PMS:
        ...
        await message.edit("`PM_guard activated`", del_in=5, log=__name__)
    else:
        Config.ALLOW_ALL_PMS = True
        await message.edit("`PM_guard deactivated`", del_in=5, log=__name__)
        pmCounter.clear()
...
async def set_custom_nopm_message(message: Message):
    ...
    if "-r" in message.flags:
        await message.edit("`Custom NOpm message reset`", del_in=5, log=True)
        noPmMessage = bk_noPmMessage
        await SAVED_SETTINGS.find_one_and_delete({"_id": "CUSTOM NOPM MESSAGE"})
    else:
        string = message.input_or_reply_raw
        if string:
            await message.edit("`Custom NOpm message saved`", del_in=5, log=True)
            noPmMessage = string
            await SAVED_SETTINGS.update_one(
...
async def set_custom_blockpm_message(message: Message):
    ...
    if "-r" in message.flags:
        await message.edit("`Custom BLOCKpm message reset`", del_in=5, log=True)
        blocked_message = bk_blocked_message
        await SAVED_SETTINGS.find_one_and_delete({"_id": "CUSTOM BLOCKPM MESSAGE"})
    else:
        string = message.input_or_reply_raw
        if string:
            await message.edit("`Custom BLOCKpm message saved`", del_in=5, log=True)
